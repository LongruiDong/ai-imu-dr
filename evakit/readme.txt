...
The base directory of each folder additionally contains: //每个序列文件夹内还有相机的校准信息
...
transform it like:
  ...
  x = Pi * Tr * X //这里的X是在laser scanner 坐标系下的坐标（是从世界点坐标通过转换矩阵得到），Pi是从3*3的相机内参矩阵增了1列得到，因为后面是齐次坐标

times.txt: Timestamps for each of the synchronized image pairs in seconds,
in case your method reasons about the dynamics of the vehicle. //每个图像对的时间戳信息

Folder 'poses': //是类似于 T_wc 实现 cam to world ,所以result也是T_wc
...
format. //转换矩阵按行铺开，只是左相机的（cam 0）,第i个位姿是针对于第0帧的？
